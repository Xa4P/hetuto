...
### Undiscounted ICERs
Calculate the **undiscounted** ICERS for each of these interventions using the total incremental costs and effects you calculated in the previous step, or using the original `df_thx` object. Suggestion: store the results in the `v_icer_undisc` for instance. `v_res_undisc` contains the total **undiscounted** costs and QALYs for each strategy.

```{r cacl_icer_undisc, exercise = TRUE}
v_res_undisc <- colSums(df_thx[, c(2:ncol(df_thx))]) # contains the total UNDISCOUNTED costs and QALYs
...
```{r plot_v_disc_serv, context = "server", echo = TRUE}

output$plot_v_disc <- renderPlot({
  v_disc <- 1/(1 + input$r_disc1) ^ df_thx$Year # create vector of discount weights
...
Now increase the discount rate to 10% and decrease it to 1% in the graph above, can you explain what happens with the discount weight over time? Can you explain what it means for the discounted costs and effects?
```{r, echo = FALSE}
...
# define v_res_disc and v_icer_disc
```
