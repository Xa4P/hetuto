...
This part of the assignment aims at defining the different parameters of the model. In the following R code chunk, you can see the different parameters of the decision tree.    
...
Calculate `p_Stable`, `p_DeathClipping` and `p_DisabledCoiling`, since these are set to 0 for now...  
Use the decision tree diagram displayed on the previous tab and the above-defined probabilities
...
```{r calc_probs-hint}
"Use the <- to assign the values. remember that the sum of probabilities stemming from a decision node should be equal to 1."
```
...
Calculate the probabilities of each pathway using the above probabilities defined in the previous step.  
