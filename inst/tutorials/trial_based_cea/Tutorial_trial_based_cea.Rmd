...
ICER <- diff_costs / diff_qol
data.frame(
  Difference_qol = diff_qol,
  Difference_costs = diff_costs,
  ICER = ICER
)
```
...
output$det_tbl<- renderTable({
  ...
  res_tbl <- df %>%
    ...
    summarise(Mean_Qol = round(mean(Qol), 2),
              Mean_Total_Cost = format(round(mean(Total_costs), 0), big.mark = ",", scientific = FALSE),
    )
  res_tbl <- cbind(res_tbl,
                   ...
                   ICER = c("", format(round((as.numeric(as.character(res_tbl$Mean_Total_Cost[2])) - as.numeric(as.character(res_tbl$Mean_Total_Cost[1]))) / (as.numeric(as.character(res_tbl$Mean_Qol[2])) - as.numeric(as.character(res_tbl$Mean_Qol[1]))), 0), big.mark = ",", scientific = FALSE)
  )
  )
  res_tbl
  })
