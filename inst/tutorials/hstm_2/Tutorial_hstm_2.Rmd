...
## Before starting
1. Watch the videos explaining how a Health State Transition Model (HSTM) works and can be used for a cost-effectiveness analyses (CEA), on uncertainty assessment, and on the probabilistic analysis. To do so, you probably need to open open the tutorial in your browser using the 'Open in Browser' button on the upper left side of the screen.  
...
![](https://vimeo.com/453911471/b1c7ed9e88){width = "70%"}

Link to the video: https://vimeo.com/453911471/b1c7ed9e88  
...
Please select the most adequate distribution for the `r_inc_mi` input parameter. In the plot below, you can see different density distribution plotted to the mean value and standard error of this parameters.  
...
Perform the probabilistic analysis by pushing the button ("Push to perform the Probabilistic Analysis!"). Once your have pushed the button, the following panels provide:  
...
- the first 25 sets of parameters are shown, these are used in the first 25 iterations of the probabilistic analysis  
...
**To perform the probabilistic analysis:** Determine the number of simulation you want to run (at least 1,000), determine the seed number to use, and push the button to run the probabilistic analysis. You can also modify the mean and standard error of the efficacy of aspirin and starting age of the cohort. It can take some time before the results appear.  
...
tags$hr()
h3("First 25 probabilistic inputs")
...
3.	Looking at the incremental cost-effectiveness plane, what do you think is the probability that the aspirin treatment strategy will be effective (QALYs will be gained)? Note that this is calculated right of the plane.   
```{r}
textInput("Answer03", "Type your answer")
```

Above, the cost-effectiveness acceptability curves are drawn for both strategies and the probability that each strategy is cost effective is provided in the table right from the figure.  
...
7.	Investigate what would happen if you would perform a study to be more sure about the treatment effect of aspirin on MI. In order to do this, change the standard error (in the "Assignment HSTM 2C - inputs"-tab) from 0.15 to 0.02 and rerun the probabilistic analysis by pushing the button. What has happened to the clouds in the CE and ICE plane? What would you conclude from the CEAC now?  
```{r}
