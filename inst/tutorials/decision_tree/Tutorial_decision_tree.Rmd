...
digraph {
      ...
      E -> I [xlabel = 'Aneurysm is coiled']
      F -> J [xlabel = 'Patient dies']
      F -> K [label = 'Patient becomes disabled']
      F -> L [xlabel = 'Patient survives in good health']
      ...
      I -> S [xlabel = 'Patient becomes disabled']
...
digraph {
      ...
      B -> D 
      C -> E
      D -> F [xlabel = 'p_Rupture'] 
      D -> G [label = 'p_Stable']
      E -> H [label = 'p_Clipping']
      E -> I [xlabel = 'p_Coiling']
...
As you can see, the `p_Stable`, `p_DeathClipping` and `p_DisabledCoiling` are not defined. Calculate `p_Stable`, `p_DeathClipping` and `p_DisabledCoiling` using the already defined probabilities and the diagram of the decision tree diagram displayed on the previous tab.  
...
```{r calc_probs-hint}
"Use <- to assign the values. remember that the sum of probabilities following a chance node should be equal to 1."
```
...
c(
  `Probability stable` = p_Stable,
  `Probability death after clipping` = p_DeathClipping,
  `Probability disabled after Coiling` = p_DisabledCoiling) # show
``` 
...
# calculate and assign the probabilty of each path, remove the `#` to perform the calculations
#p_path_A
#p_path_B
#p_path_C
#p_path_D
...
# calculate and assign the effects of each path, remove the `#` to perform the calculations
#e_path_A 
#e_path_B  
#e_path_C 
#e_path_D 
...
# calculate and assign the costs of each path, remove the `#` to perform the calculations
#c_path_A 
#c_path_B  
#c_path_C 
#c_path_D 
...
Now that we have calculated the expected outcomes of each pathway, we can finally calculate the expected outcomes of each strategy! This can be done by summing up all expected outcomes of the pathways of each strategy. 
...
#t_e_ww <- ___ # expected effects "watchful waiting" strategy
#t_c_ww <- ___ # expected costs "watchful waiting" strategy

#t_e_trt <- ___ # expected effects treatment strategy
#t_c_trt <- ___ # expected costs treatment strategy
...
### Calculations
Based on the total expected utility value (effects) and costs of both strategies, you can now calculate the incremental effects and costs of the treatment strategy versus "watchful waiting". You can also calculate the incremental cost-effectiveness ratio of the treatment strategy versus "watchful waiting". Please do so in the following chunk
The total effects and costs defined in the previous step (`t_e_ww`, `t_c_ww`, `t_e_trt`, `t_c_trt`) have been loaded and can be used to perform these calculations.  
```{r set_up_comparison}
p_Stable <- 1 - p_Rupture 
p_DeathClipping  <- 1 - p_DisabledClipping - p_SurvivalClipping
p_DisabledCoiling <- 1- p_DeathCoiling - p_SurvivalCoiling
...
#inc_e_trt <- ___ # incremental effects
#inc_c_trt <- ___ # incremental costs
#icer <- ___ # incremental cost-effectiveness ratio
```
...
In this tab, the decision tree has been filled with the input values of the previous steps. You can change the input values and evaluate what the impact of changing these inputs is on the results. This completed decision tree can be used to answer the questions below.  
